...
int total = 0, n, m, u, v, p[N], curtime = 0, q[N];
char tmp[N];
...
int main() {
	...
	for (int i = 0, k = 0, j = 0; i < n; ++i) {
		k = j = states[j][tmp[i]];
		if (!states[k].is_terminal) k = states[k].next_terminal;
		while (k != -1) {
			set_parent(i - states[k].len + 1, i + 1);
			k = states[k].next_terminal;
		}
	}
	scanf("%d", &m);
	for (int i = 0; i < m; ++i) {
