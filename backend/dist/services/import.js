...
async function importPackage(zipPath, ownerId, overwriteExisting = false) {
    ...
    const zipStat = fs_1.default.statSync(zipPath);
    if (zipStat.size > MAX_ZIP_SIZE)
        throw new Error(`Zip too large: ${zipStat.size} bytes`);
    const zip = new adm_zip_1.default(zipPath);
    const zipEntries = zip.getEntries();
    if (zipEntries.length > 20000)
        throw new Error(`Too many files in zip: ${zipEntries.length}`);
