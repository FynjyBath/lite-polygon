...
exports.getProblemDir = getProblemDir;
...
exports.db = new Proxy({}, {
    get(_, prop) {
        if (!_real)
            throw new Error('Call initSchema() before using db');
        const val = _real[prop];
        return typeof val === 'function' ? val.bind(_real) : val;
    },
});
function initSchema(dataDir) {
    _dir = dataDir || process.env.DATA_DIR || path_1.default.join(process.cwd(), '..', 'data');
    fs_1.default.mkdirSync(_dir, { recursive: true });
    fs_1.default.mkdirSync(path_1.default.join(_dir, 'problems'), { recursive: true });
    fs_1.default.mkdirSync(path_1.default.join(_dir, 'packages'), { recursive: true });
    if (_real) {
        try {
            _real.close();
        }
    ...
    _real = new better_sqlite3_1.default(path_1.default.join(_dir, 'db.sqlite3'));
  ...
  `);
    ...
    const count = _real.prepare('SELECT COUNT(*) as c FROM users').get().c;
    if (count === 0) {
        const bcrypt = require('bcryptjs');
        const hash = bcrypt.hashSync('admin', 10);
        _real.prepare("INSERT INTO users (username, password_hash, must_change_password) VALUES (?, ?, 1)").run('admin', hash);
    }
}
function getDataDir() {
    return _dir;
}
function getProblemDir(problemId) {
    return path_1.default.join(_dir, 'problems', String(problemId));
}
function getPackagesDir() {
    return path_1.default.join(_dir, 'packages');
}
